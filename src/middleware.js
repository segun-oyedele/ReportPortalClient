...
export function middleware(req) {
    ...
    if(req.nextUrl.pathname.includes('/auth/login')) {
        if(!!authCookie) {
            return NextResponse.redirect(new URL('/Xcelerator/cdl-report-portal/', req.url));
        }
    }
    ...
    if(req.nextUrl.pathname.includes('/detailed-report')) {
        if(!authCookie) {
            return NextResponse.redirect(new URL('/Xcelerator/cdl-report-portal/auth/login', req.url));
        }
    }
