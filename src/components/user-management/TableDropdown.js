import { setUserToDelete, setOpenUserDeleteForm } from '@/store/user-management';
...
export const TableDropdown = ({handleEditUser, user}) => {
      ...
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex justify-center w-full py-2 text-sm font-medium rounded-md hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
            <PencilAltIcon className="w-6 h-6 text-black" />
          </Menu.Button>
        </div>
        <Transition
          as={Fragment}
        ...
        >
          <Menu.Items className="absolute right-0 z-10 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg dropdown__group ring-1 ring-black ring-opacity-5 focus:outline-none">
                ...
                {({ active }) => (
                  <button
                    ...
                    onClick={ () => {
                      ...
                      dispatch(setOpenUserDeleteForm(true))
                    } }
                  >
                    <Image
