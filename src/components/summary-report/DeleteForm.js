...
import { setOpenGroupDeleteForm } from '@/store/summary-report';
...
export const DeleteForm = ({ handleClick }) => {
  ...
  const { groupToDelete } = useAppSelector( state => state.summaryReport );
  ...
  return (
    ...
    <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-slate-200/50">
      <div className="w-full max-w-md bg-white shadow form__container delete__form">
        ...
        >
          Are you sure you want to delete the <span className="raleway-eb">{ groupToDelete.report_name }</span> group?
        </PageDescription>
