...
export const TableBody = memo(
  ({
    ...
    return (
      <tbody>
        {sorted.map(({ order_tracking_id, driver_no, days_old }, index) => (
          <tr
          ...
          >
            ...
            >
              ...
              <a
                href={`https://ship.cdldelivers.com/Xcelerator/Orders/ReviewOrder/ReviewOrder?_p_Odata=${order_tracking_id}`}
                className="raleway-m cursor-pointer"
                target="_blank"
              >
                {order_tracking_id}
              </a>
            ...
            <td
              className="px-6 text-base text-center truncate max-h-16 table_text-black hidden sm:block"
              title={driver_no}
            >
              {driver_no}
            </td>
