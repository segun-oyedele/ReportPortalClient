...
const LoginPage = () => {
  ...
  const { formValues, handleEmailChange, handlePasswordChange, emailError, passwordError } = useForm(initialForm);
  const { email, password } = formValues;
  const formRef = useRef(null);

  const showButton = !emailError && !!email.length && !passwordError && !!password.length;
