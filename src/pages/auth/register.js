...
import { GoBackButton, InputField, LockIcon, PageDescription, PageTitle } from '@/shared/components';
...
const RegisterPage = () => {
  ...
  const { formValues, handleEmailChange, handlePasswordChange, handleNameChange, emailError, passwordError, firstNameError, lastNameError } = useForm(initialForm);
  const { email, password, first_name, last_name } = formValues;
  const showButton = !emailError && !!email.length && !passwordError && !!password.length && !firstNameError && !!first_name.length && !lastNameError && !!last_name.length && (email.split('@')[1] === 'cdldelivers.com' || email.split('@')[1] === 'broadviewtechnicalsolutions.com')
